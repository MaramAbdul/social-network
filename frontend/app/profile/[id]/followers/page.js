...
export default function FollowersPage() {
  ...
  const { users, loading, error } = useFollowList(id, "followers");
      ...
      <div className="max-w-2xl mx-auto p-6">
        ...
        {error ? (
          <div className="bg-card rounded-xl p-6 border border-border text-center">
            ...
            <h2 className="text-lg font-semibold text-foreground mb-2">Unable to Load Followers</h2>
            <p className="text-muted">{error}</p>
          </div>
        ) : users.length === 0 ? (
          <div className="bg-card rounded-xl p-6 border border-border text-center">
            <div className="text-4xl mb-2">👥</div>
            <h2 className="text-lg font-semibold text-foreground mb-2">No Followers</h2>
            <p className="text-muted">This user doesn't have any followers yet</p>
          </div>
        ) : (
          <div className="space-y-4">
            {users.map((user) => (
